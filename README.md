...
完整的部署回归顺序、核心功能验收项、安全/性能/恢复门禁见 [完整测试流程与核心功能验收](docs/完整测试流程与核心功能验收.md)。

## 当前交付状态

- 当前代码目标迁移版本：`0025_stock_document_versions`。
- 正式数据库版本：`0011_livestock_production`；`0012` 至 `0024` 仅在隔离库验证，正式升级按约定留到其余功能完成后统一执行，日常业务账号仍仅保留业务读写权限。
...
- 阶段 8 状态：已完成；已交付农场综合驾驶舱、销售利润 CSV 导出、任务提醒、任务完成流转、审计查询和受控附件存储/下载。
- V1 核心交付总进度：约 98%；阶段 1–3、阶段 5–8 已完成，阶段 4 约 85%，阶段 9 已完成主要加固；内置智能体已进入采购受控写入阶段。
- 阶段 9 状态：进行中（约 35%）；已增加 `release-check` 配置/Schema 门禁、Windows 发布检查脚本和 Nginx 权限策略响应头。
- 阶段 9 状态：进行中（约 98%）；已增加备份 SHA-256/GZip 完整性校验、仅允许 `_test` 数据库的恢复演练和 MySQL 迁移演练脚本、发布配置门禁、迁移链/敏感文件校验、健康接口性能烟测、前端 ESLint/Vitest 和销售经营对账发布门禁。完整桌面/移动端 E2E 仍需发布前人工回归。
- 服务器测试部署：已在隔离目录和 `agriculture_management_test` 完成 `0011_livestock_production → 0023_attachments` 升级；`0024_agent_confirmation_nonces` 与 `0025_stock_document_versions` 需重新执行测试库迁移，正式服务 `127.0.0.1:5000` 未切换。
...
- 内置智能体状态：安全受控链路已完成约 95%；已交付后端短时用户令牌、角色/农场隔离、独立 Harness runtime 网关、网关作用域工具凭证、前端智能体入口、采购/盘点草稿、持久化一次性确认 nonce、过账幂等和审计闭环，并已通过真实模型链路回归；删除、批量写入和无人值守自动过账仍关闭。
- 当前项目整体完成度：约 94%；其中农牧业务 V1 约 98%，内置智能体约 90%。
- 当前质量门禁：Ruff、pytest 65 项通过、1 项按配置跳过；ESLint、Vitest 4 项、Vue 类型检查和 Vite 构建通过；Harness 网关/插件 TypeScript 检查和真实模型链路回归通过。正式库仍停在 `0011`，不使用当前代码启动正式服务。
- 已记录待补齐项：管理员 API 管理功能（供应商、API 地址、模型、密钥的服务端安全配置、连接测试、切换和审计），详见 [智能体待办清单](docs/智能体待办清单.md)。

## 后续优化优先级

1. 扩展内置智能体的盘点、调拨和生产记录草稿工具，继续保持确认门禁。
2. 增加固定问集、模型调用观测和提示注入回归，完善前端草稿预览与确认状态。
3. 实现管理员 API 管理功能，完成密钥掩码、连接测试、配置切换、审计和网关重载。
4. 取得真实出栏/死亡重量后，将当前估算 FCR 升级为实算口径。
5. 优化手机快速录入，并按实际使用反馈评估 PWA、微信入口或弱网能力。
