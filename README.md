...
- 当前代码目标迁移版本：`0023_attachments`。
- 正式数据库版本：`0011_livestock_production`；`0012` 至 `0023` 仅在隔离库验证，正式升级按约定留到其余功能完成后统一执行，日常业务账号仍仅保留业务读写权限。
...
- V1 核心交付总进度：约 98%；阶段 1–3、阶段 5–8 已完成，阶段 4 约 85%，阶段 9 尚待开发，智能体保持只读试验状态。
...
- 服务器测试部署：已在隔离目录和 `agriculture_management_test` 完成 `0011_livestock_production → 0023_attachments` 升级，测试服务监听 `127.0.0.1:15000`；结构检查、库存/销售对账、登录与关键只读接口烟测通过，正式服务 `127.0.0.1:5000` 未切换。
