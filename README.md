...
- 当前代码目标迁移版本：`0022_workflow_audit`。
- 正式数据库版本：`0011_livestock_production`；`0012` 至 `0022` 仅在隔离库验证，正式升级按约定留到其余功能完成后统一执行，日常业务账号仍仅保留业务读写权限。
...
- 阶段 8 状态：进行中（约 75%）；已交付农场综合驾驶舱、销售利润 CSV 导出、任务提醒、任务完成流转和审计查询，附件安全仍待完成。
- V1 核心交付总进度：约 96%；阶段 1–3、阶段 5–7 已完成，阶段 4 约 85%，阶段 8 约 75%，阶段 9 尚待开发，智能体保持只读试验状态。
