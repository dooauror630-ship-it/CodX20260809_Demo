...
当前阶段启用七个只读工具、三个受控草稿工具和三个受控确认工具：当前用户、可访问农场、采购基础选项、管理员用户列表、库存概览、养殖概览、销售经营汇总，以及采购入库、库存盘点、仓库调拨草稿和对应的确认过账。Harness 不连接数据库，草稿只创建 DRAFT 业务单据；正式过账必须提交后端签发的短时确认挑战，后端仍二次校验用户角色、农场范围、库存快照和单据版本。
