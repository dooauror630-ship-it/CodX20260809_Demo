...
| 质量门禁 | 已建立 | Ruff；pytest 29 项通过、1 项按配置跳过；MySQL 集成 1 项通过；Vitest 4 项通过；ESLint、Vue 类型检查和 Vite 构建通过；桌面与移动 Playwright 6 项通过；`0011` 在隔离 MySQL 和正式库真实迁移通过；正式结构检查、库存余额对账、健康接口和 Vue 入口通过 | 后续模块继续按风险增加固定数据集和业务闭环测试 |

截至本次更新，正式 MySQL 迁移版本为 `0011_livestock_production`，新增生猪健康/用药和称重记录表，并将库存领退料成本对象扩展到养殖批次。当前存栏与圈舍分布继续由追加式流水实时计算。正式升级前无 GTID 备份位于 `backups/agriculture_management-before-0011-release-20260817-201048.sql`，SHA256 为 `C1A3DF000C2B67B1951B4C4BDD12E2015607B05EE727C70F6A51545FD711966C`。隔离测试库和正式库均通过结构检查，正式库存余额通过流水对账；写入型自动化数据仅进入名称以 `_test` 结尾的测试库。
...
| 4. 生猪管理 | 进行中 | 批次、存栏、饲喂领料、健康/防疫/用药、称重和 ADG 已完成；继续开发 FCR 和完整批次成本 | `0010_livestock`、`0011_livestock_production` 已上线 | 当前存栏、减员/出栏 KPI、最近均重、ADG 和饲料成本已完成；趋势、死亡率、FCR、完整成本待开发 | 使用真实示例完整关闭一批猪；库存、存栏、成本和销售可追溯 |
