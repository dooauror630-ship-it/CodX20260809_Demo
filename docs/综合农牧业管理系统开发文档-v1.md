...
| 质量门禁 | 已建立 | Ruff；pytest 29 项通过、1 项按配置跳过；MySQL 集成 1 项通过；Vitest 4 项通过；ESLint、Vue 类型检查和 Vite 构建通过；桌面与移动 Playwright 6 项通过；`0011` 隔离 MySQL 真实迁移和结构检查通过；浏览器脚本异常、非预期 HTTP 失败、图表像素和响应式截图检查通过 | 正式库取得一次性 DDL 账号后执行 `0011` 发布、结构检查和余额对账 |
