...
| 阶段 4：生猪管理 | 进行中（批次与存栏基础完成） | `0010_livestock`；批次入栏、转舍、死亡、淘汰、出栏、圈舍分布、当前存栏、容量校验、自动结批、幂等、业务日期顺序、角色权限和农场隔离 | 饲喂、防疫、用药、称重、批次成本和养殖分析 |
...
| `0011_crop_tobacco`（规划） | 种植、采收、分级、烟草烘烤 | 测试面积和产量约束 |
| `0012_trade_finance`（规划） | 销售、收付款和成本 | 测试销售、库存和利润联动 |
| `0013_tasks_attachments`（规划） | 任务、附件和扩展审计 | 测试文件安全和提醒幂等 |
| `0014_analytics_summary`（按需） | 仅在触发性能条件时增加汇总表 | 与原始业务表逐项对账 |
...
| 4. 生猪管理 | 进行中 | 批次、入栏、转舍、死亡、淘汰、出栏和自动结批已完成；继续开发饲喂、防疫、用药、称重和成本 | `0010_livestock` 已上线；后续按实际字段增量迁移 | 当前存栏、在养批次、累计死亡及淘汰/出栏 KPI 已完成；趋势、死亡率、ADG、FCR、成本待开发 | 使用真实示例完整关闭一批猪；库存、存栏、成本和销售可追溯 |
